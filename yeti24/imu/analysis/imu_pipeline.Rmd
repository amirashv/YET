---
title: "IMU Head Motion Prototype — Gyroscope Processing Pipeline"
...
This document describes the gyroscope preprocessing pipeline implemented for the IMU module of the YETI24 low-cost eye-tracking prototype. The IMU is used to estimate relative head orientation, which can later be combined with eye-camera data for motion compensation and improved gaze stability [@larsson2014].
...
This report uses R (≥4.0) and packages: tidyverse. Install once via install.packages("tidyverse").
...
2.  Stationary bias recording (D_2): the same sensor placed still, used to estimate the gyroscope offset (“bias”) that is present even when the true angular velocity is zero.

This separation is necessary because gyroscopes typically output a small non-zero value at rest, and integrating this offset over time would cause large drift in estimated orientation [@ji-hoon2025].
...
All later steps depend on the time step Δt between samples [@woodman2007, @schwarzmann2025, @yang2021] :

-   Bias-corrected filtering uses Δt to set the smoothing strength consistently.

-   Integration from angular velocity to angles uses Δt directly (angles = ∑ ω · Δt).

-   Irregular sampling (“jitter”) increases error and makes drift worse.

Therefore, we compute Δt from microcontroller timestamps and summarise the mean sampling interval, variability, and the implied sampling frequency (Hz).
...
Any irregularity in Δt directly propagates into the orientation estimate.

## Computation of the Δt

Δt is computed as the difference between consecutive timestamps. The summary table reports:

-   Mean Δt: average time between samples.

-   SD(Δt) and CV (coefficient of variation): stability of sampling.

-   fs_mean_hz: estimated sampling rate (1 / mean Δt).
...
cat(sprintf("Mean Δt: %.6f s; SD(Δt): %.6f s; CV: %.3f\n", dt_mean, dt_sd, cv))

```

## Visualization of the Δt Distribution

The histogram of Δt serves as a diagnostic tool for assessing sampling stability. Ideally, Δt values cluster tightly around a single value, indicating a stable sampling rate.

If the histogram shows a long right tail (i.e., occasional larger Δt values), this suggests delayed or irregular sampling. Such delays can lead to exaggerated integration steps (θ = ∑ ω·Δt), potentially producing spikes in estimated angles and reducing filter reliability. Therefore, inspecting this distribution is essential before proceeding with filtering and integration.

```{r}
ggplot(D_1_dt, aes(x = dt_s)) +
  geom_histogram(bins = 50) +
  labs(
    title = "Δt distribution (from microcontroller timestamps)",
    x = "Δt (seconds)",
    y = "Count"
  )
```
...
When the sensor is stationary (ω_true = 0), the measured output approximates the bias term. Therefore, the bias is estimated as: $$b = \frac{1}{N} \sum_{i=1}^{N} \omega_i $$
...
After bias subtraction, the stationary gyroscope means should be close to zero on all axes. Small residual offsets are normal (finite sample + noise), but large residual means would suggest either an incorrect bias estimate or that the “stationary” recording contained movement.
...
The EMA is controlled by α [@haugen2008]. We define it using a time constant τ:

-   τ sets the “memory” of the filter (larger τ → smoother but slower response).

-   α converts τ into a per-sample smoothing weight.

The coefficient is derived from the time constant τ: $$\alpha = \frac{\tau}{\tau + \Delta t}$$
...
The EMA outputs a filtered signal y where each new value is a weighted combination of the previous filtered value and the current observation. This reduces rapid noise fluctuations while preserving the general movement trend, which improves the stability of later angle integration [@ducard2018].
...
Larger τ increases smoothing but reduces responsiveness.
...
We compute roll/pitch/yaw in radians via cumulative summation of ω · Δt, then convert to degrees for interpretability. This is a standard numerical integration approach for discrete sensor samples.
...
The plot shows the integrated roll/pitch/yaw angles over time. It is normal for the yaw (often corresponding to the Z axis depending on sensor mounting) to look less stable and to drift more, because yaw is typically most sensitive to small bias and cannot be “corrected” without an external reference (e.g., accelerometer/magnetometer or optical constraints). Therefore, the yaw curve should be interpreted cautiously, especially over longer durations.
